...
def normalize_layer_composition(layer_counts, mode='within_layer',
                                zero_policy='nan'):
    """Normalize mapped counts within each layer or across all layers per ROI."""
    if mode not in {'within_layer', 'within_region'}:
        raise ValueError("mode must be 'within_layer' or 'within_region'")
